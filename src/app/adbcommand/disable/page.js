...
export default function adbcommand() {
  ...
  const [closingbluetooth, setclosingbluetooth] = useState(false);
  ...
  const handleConfirm = () => {
    if (actionType === 'Disable') {
      handleDisabledevice(modaldeviceIp);
    }else if (actionType === 'Disableall') {
      handleDisablealldevice(modaldeviceIp);
    }else if (actionType === 'Enable') {
      handleEnabledevice(modaldeviceIp);
    }else if (actionType === 'Enableall') {
      handleEnablealldevice(modaldeviceIp);
    }else if (actionType === 'closingbluetooth') {
      handleClosebluetooth(devices);
    }
     else {
...
const handleEnabledevice = async (ip) => {
  try {
    ...
    const response = await fetch('/api/adbcommand/adbenableapp', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const handleDisabledevice = async (ip) => {
    try {
      // ตรวจสอบว่า packageName ถูกกำหนดไว้หรือยัง
      if (!packageName) {
        setLogMessage('กรุณาเลือกแพ็กเกจก่อน');
      ...
      // เรียก API เพื่อเปิดแอป
      const response = await fetch('/api/adbcommand/adbdisableapp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify([{ ip, packageName: packageName }]), // ส่งข้อมูล IP และ package ที่เลือก
      });
...
const handleEnablealldevice = async () => {

  if (!packageName) {
    // หากยังไม่ได้เลือก packageName
    setDevices(prevDevices =>
      prevDevices.map(device => ({
        ...device,
        status: 'กรุณาเลือก package'  // อัปเดต status ของทุก device
      }))
    );
    setLogMessage('กรุณาเลือก package ก่อน');
    return; // หยุดการทำงาน
  }

  setenabling(true); // เริ่มการติดตั้ง
  ...
  setenabling(false); // เสร็จสิ้น
};

const handleDisablealldevice = async () => {
  ...
  setdisabling(true); // เริ่มการติดตั้ง
  const devicesData = devices.map(device => ({
    ip: device.ip,
    packageName: packageName, // เพิ่ม packageName เข้าไปใน JSON
  }));
  ...
  setdisabling(false); // เสร็จสิ้น
};


const handleClosebluetooth = async () => {
 

  setclosingbluetooth(true); // เริ่มการติดตั้ง
  const devicesData = devices.map(device => ({
    ip: device.ip,
  }));

  const response = await fetch('/api/adbcommand/adbdisablebluetooth', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(devicesData), // ส่งเฉพาะ ip ของอุปกรณ์
  });
  ...
  setclosingbluetooth(false); // เสร็จสิ้น
};
...
useEffect(() => {
    ...
    if (packageName) {
      ...
      console.log("device after find package: ", devices,packageName)
      setDevices([...devices]);
...
return (
  <div>
   ...
   Disable / Enable
        ...
        <p>
          ⚠️ การกด <strong>Disable App</strong><br></br>อาจทำให้เครื่องมีปัญหาหรือเกิดผลกระทบกับการทำงานของแอปพลิเคชัน ควรเลือกเฉพาะแอปที่ต้องการเท่านั้น
        </p>
      </div>
  ...
  <td colSpan="7" className="text-center p-4 align-middle">กำลังโหลดข้อมูล...</td>
            ...
            devices.map((device, index) => (
              <tr key={index} className={`${index % 2 === 0 ? "bg-gray-100" : "bg-white"} hover:bg-gray-200`}>
                ...
                <td className="border px-4 py-3 text-center">
                  <button
                    onClick={() => handleOpen('Disable',device.ip)}
                ...
                <td className="border px-4 py-3 text-center">
                  <button
                    onClick={() => handleOpen('Enable',device.ip)}
                    sx={{ ...modalbuttonStyle, ...modalhoverButtonStyle, backgroundColor: '#f57c00', color: 'white' }}
...
<Modal
    ...
    <p>
      📱 หน้านี้ช่วยให้คุณสามารถจัดการกับการเปิดใช้งานและปิดใช้งานแอปพลิเคชันที่ติดตั้งในอุปกรณ์ที่เชื่อมต่อผ่าน ADB ได้ โดยการเลือก <strong>Disable</strong> หรือ <strong>Enable</strong> สำหรับ package ที่ต้องการ.<br />
      ...
      4. หลังจากเลือกแอปแล้ว คุณสามารถเลือกการดำเนินการที่ต้องการ ได้แก่ <strong>Disable</strong> หรือ <strong>Enable</strong> สำหรับแอปนั้น.<br />
      5. คุณสามารถเลือกที่จะดำเนินการกับแอปบนอุปกรณ์เฉพาะเครื่อง หรือทำการดำเนินการกับอุปกรณ์ทั้งหมดที่เชื่อมต่อ.<br />
      โปรดระมัดระวังในการเลือกแอปที่ต้องการจัดการเพื่อหลีกเลี่ยงผลกระทบต่อการทำงานของอุปกรณ์.
    </p>
  </div>
</div>
  </div>

...
export async function POST(req) {
  try {
    // รับข้อมูลจาก body
    const {apkpatk, ip} = await req.json();  // ดึงข้อมูลทั้งหมดจาก body (รวมถึง ip, mac, hostname)
    
    if (!apkpatk || apkpatk.length === 0) {
      return new Response(
        JSON.stringify({ message: 'กรุณาระบุข้อมูล path apk' }),
        { status: 400 }
      );
    }
    if (!ip || ip.length === 0) {

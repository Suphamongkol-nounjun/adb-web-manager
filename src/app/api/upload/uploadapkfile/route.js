...
export const GET = async (req) => {
  ...
  if (!adbUuid) {
    return NextResponse.json({ message: "ไม่พบ adb-uuid" }, { status: 400 });
  }

  const directoryPath = path.join(process.cwd(), 'src', 'platform-tools', 'adb', adbUuid); // ใช้ adb-uuid เป็นชื่อโฟลเดอร์

  try {

    try {
      await access(directoryPath, constants.F_OK); // F_OK ตรวจสอบว่าไฟล์หรือโฟลเดอร์นั้นมีอยู่
    } catch (error) {
      // ถ้าไม่มีโฟลเดอร์, สร้างใหม่
      await mkdir(directoryPath, { recursive: true });
      console.log(`โฟลเดอร์ ${directoryPath} ถูกสร้างขึ้น`);
    }
  ...
  export const POST = async (req, res) => {
    ...
    const adbUuid = formData.get("adb-uuid"); // รับค่า adb-uuid จาก formData
    ...
    if (!adbUuid) {
      return NextResponse.json({ error: "adb-uuid is required." }, { status: 400 });
    }
    ...
    try {
      // ตรวจสอบและสร้างโฟลเดอร์สำหรับ adb-uuid ถ้ายังไม่มี
      await mkdir(directoryPath, { recursive: true });

...
export async function POST(req) {
  try {
      ...
      start(controller) {
        (async () => {
          for (let device of devices) {
            ...
            console.log(`🌐 กำลัง Enable แอป ${packageName} บนอุปกรณ์ที่มี IP: ${ip}`);

            try {
              const result = await new Promise((resolve) => {
                exec(
                  `"${adbPath}" -s ${ip} shell pm enable ${packageName}`,
                  (err, stdout, stderr) => {
                    let status = 'Enable แอป ไม่สำเร็จ';
                    let message = `ไม่สามารถ Enable แอป ${packageName} บนอุปกรณ์ที่ ${ip} ได้`;
                    ...
                    if (stderr.includes("Unknown package")) {
                      ...
                      status = 'Enable แอป สำเร็จ';
                      message = `Enable แอป ${packageName} บนอุปกรณ์ที่ ${ip} สำเร็จ`;
                    }
              ...
              const errorMessage = JSON.stringify({
                ip: device.ip,
                status: 'Enable แอป ไม่สำเร็จ',
                message: `ไม่สามารถ Enable แอป ${packageName} บนอุปกรณ์ที่ ${ip} ได้`,
    ...
    return new Response(
      JSON.stringify({
        status: 'error',
        message: 'เกิดข้อผิดพลาดในการ Enable แอป',
        error: error.message,
      }),
      { status: 500 }
    );
